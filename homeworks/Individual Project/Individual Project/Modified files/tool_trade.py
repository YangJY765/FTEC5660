...
def buy(symbol: str, amount: int) -> Dict[str, Any]:
    ...
    from agent_tools.tool_get_price_local import get_price_local_daily
    price_data = get_price_local_daily(symbol, today_date)
    
    if "error" in price_data:
        return {
            "error": f"Symbol {symbol} not found or price missing! Detail: {price_data['error']}",
            "symbol": symbol,
            "date": today_date,
        }
    this_symbol_price = float(price_data["ohlcv"]["close"])
